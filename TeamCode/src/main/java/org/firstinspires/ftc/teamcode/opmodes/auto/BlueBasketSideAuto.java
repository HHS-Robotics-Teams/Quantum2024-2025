...
public class BlueBasketSideAuto extends OpMode {
    ...
    /*
        EVERY SINGLE LOCATION FOR ALL AUTONOMI WILL BE BELOW
        BEWARNED MY LAZINESS                                          */
    Pose startPose = new Pose(8, 88.2, Math.toRadians(180));
    Point blueBasketStart = new Point(8, 88.2, Point.CARTESIAN);
    Point blueBasketSpeceminDrop = new Point(27, 80.5, Point.CARTESIAN);
    Point blueBasketDrop = new Point(20, 132, Point.CARTESIAN);
    Pose blueBasketScor = new Pose(20, 129, Math.toRadians(135));
    ...
    public void start() {
        ...
        //Place
        while (!((RobotComponents.left_slide_motor.getCurrentPosition() - RobotComponents.left_slide_motor.getTargetPosition()) < 20)) {
            try {
                wait(1);
    ...
    public void buildPaths() {
        ...
        pickupTwo = follower.pathBuilder().addPath(new BezierCurve(blueBasketDrop, blueBasketScore)).setLinearHeadingInterpolation(Math.toRadians(135), Math.toRadians(150)).build();
        ...
        pickupThree = follower.pathBuilder().addPath(new BezierCurve(blueBasketDrop, blueBasketScore)).setLinearHeadingInterpolation(Math.toRadians(135), Math.toRadians(210)).build();
